...
func TestPlayerPlayGemCard(t *testing.T) {
  ...
  assert.NotEqual(t, err, nil, "Player should not be able to play a card they don't have")

  player_1.AddCard(blue_to_many)
  err = player_1.PlayGemCard(blue_to_many, GemValues{})
  assert.NotEqual(t, err, nil, "Player should not be able to end a turn with too many gems")
  assert.Equal(t, GemValues{Blue: 10}, player_1.Gems, "player gem values should be reset after illegal move")
  ...
  assert.Equal(t, err, nil, "When discarding appropriately, there should be no error")
